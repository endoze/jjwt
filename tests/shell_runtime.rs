...
impl Jj for FakeJj {
    ...
    fn workspace_add(&self, _r: &Path, n: &str, p: &Path) -> Result<()> {
        self.calls.borrow_mut().push(format!("workspace_add {n} {}", p.display()));

        Ok(())
    }

    fn workspace_forget(&self, _r: &Path, n: &str) -> Result<()> {
        self.calls.borrow_mut().push(format!("workspace_forget {n}"));

        Ok(())
    }

    fn workspace_update_stale(&self, _r: &Path, n: &str) -> Result<()> {
        self.calls.borrow_mut().push(format!("workspace_update_stale {n}"));

        Ok(())
    }

    fn bookmark_create(&self, _r: &Path, n: &str, w: &str) -> Result<()> {
        self.calls.borrow_mut().push(format!("bookmark_create {n} {w}"));
...
impl Fs for FakeFs {
    ...
    fn remove_dir_all(&self, p: &Path) -> Result<()> {
        ...
        self.calls.borrow_mut().push(format!("remove_dir_all {}", p.display()));
...
impl Proc for FakeProc {
    fn run_sh(&self, cmd: &str, cwd: &Path, _env: &[(String, String)]) -> Result<ProcOutput> {
        self.calls.borrow_mut().push(format!("sh -c {cmd} (cwd={})", cwd.display()));

        if self.fail_on.as_deref() == Some(cmd) {
            return Ok(ProcOutput { status: 1, stdout: String::new(), stderr: "boom".into() });
        }

        Ok(ProcOutput { status: 0, stdout: String::new(), stderr: String::new() })
    }
}
...
fn execute_runs_actions_in_order() {
    ...
    let plan = Plan {
        actions: vec![
            Action::JjWorkspaceAdd { name: "x".into(), path: PathBuf::from("/repo/.worktrees/x") },
            Action::JjBookmarkCreate { name: "x".into(), workspace: "x".into() },
            Action::RunHook {
                name: "h".into(),
...
fn execute_halts_on_hook_failure() {
    let mut rt = Runtime::new(
        FakeJj::default(),
        FakeFs::default(),
        FakeProc { fail_on: Some("bad".into()), ..Default::default() },
    );
    let plan = Plan {
        actions: vec![
            Action::RunHook {
    ...
    assert!(msg.contains("bad-hook"), "error should name the failing hook: {msg}");
}
