...
fn cfg_with_hooks() -> Config {
    ...
    g2.insert("db".to_string(), "make db-start in {{ branch }}".to_string());
...
fn resolves_hook_from_pre_start() {
    ...
    let args = HookArgs { name: "db".into(), current_workspace: "feat-x".into() };
    let plan = plan_hook(&cfg, &args, &obs()).expect("plan ok");
    assert_eq!(plan.actions.len(), 1);
    let Action::RunHook { name, rendered_cmd, cwd, env } = &plan.actions[0] else { panic!() };
    assert_eq!(name, "db");
    assert_eq!(rendered_cmd, "make db-start in feat-x");
    assert_eq!(*cwd, PathBuf::from("/repo/.worktrees/feat-x"));
    assert!(env.iter().any(|(k, v)| k == "JJWT_WORKSPACE" && v == "feat-x"));
}

#[test]
fn resolves_hook_from_pre_remove() {
    let cfg = cfg_with_hooks();
    let args = HookArgs { name: "db_stop".into(), current_workspace: "feat-x".into() };
    let plan = plan_hook(&cfg, &args, &obs()).expect("plan ok");
    let Action::RunHook { rendered_cmd, .. } = &plan.actions[0] else { panic!() };
...
fn missing_hook_errors() {
    let cfg = cfg_with_hooks();
    let args = HookArgs { name: "nope".into(), current_workspace: "feat-x".into() };
...
fn missing_current_workspace_errors() {
    let cfg = cfg_with_hooks();
    let args = HookArgs { name: "db".into(), current_workspace: "nope".into() };
    let err = plan_hook(&cfg, &args, &obs()).unwrap_err();
