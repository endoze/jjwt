...
fn create_emits_workspace_then_bookmark_then_hooks_then_print() {
    ...
    let args = SwitchArgs { name: "feat-x".into(), create: true, rerun_hooks: false };
    let obs = observed_clean();

    let plan = plan_switch(&cfg, &args, &obs).expect("plan ok");
    let ws_path = PathBuf::from("/repo/.worktrees/feat-x");

    assert_eq!(plan.actions[0], Action::JjWorkspaceAdd {
        name: "feat-x".into(), path: ws_path.clone(),
    });
    assert_eq!(plan.actions[1], Action::JjBookmarkCreate {
        name: "feat-x".into(), workspace: "feat-x".into(),
    });

    let Action::RunHook { name: n0, rendered_cmd: c0, cwd: cwd0, env: env0 } = &plan.actions[2] else {
        panic!("expected RunHook");
    };
    assert_eq!(n0, "direnv");
    assert_eq!(c0, "direnv allow .");
    assert_eq!(cwd0, &ws_path);
    assert!(env0.iter().any(|(k, v)| k == "JJWT_WORKSPACE" && v == "feat-x"));
    assert!(env0.iter().any(|(k, _)| k == "JJWT_WORKSPACE_PATH"));

    let Action::RunHook { name: n1, rendered_cmd: c1, .. } = &plan.actions[3] else {
        panic!("expected RunHook");
    };
    assert_eq!(n1, "envrc");
    assert_eq!(c1, "echo feat-x", "template must be rendered with branch=feat-x");

    let Action::RunHook { name: n2, rendered_cmd: c2, .. } = &plan.actions[4] else {
        panic!("expected RunHook");
    };
    assert_eq!(n2, "db");
    assert_eq!(c2, "make db-start");

    assert_eq!(plan.actions[5], Action::PrintLine(ws_path.display().to_string()));
...
fn switch_existing_no_create_emits_only_print() {
    let cfg = cfg_with_two_pre_start_groups();
    let args = SwitchArgs { name: "feat-x".into(), create: false, rerun_hooks: false };
    let mut obs = observed_clean();
    obs.workspaces.push(Workspace {
        name: "feat-x".into(),
        path: PathBuf::from("/repo/.worktrees/feat-x"),
    ...
    let plan = plan_switch(&cfg, &args, &obs).expect("plan ok");
    assert_eq!(plan.actions, vec![Action::PrintLine(
        PathBuf::from("/repo/.worktrees/feat-x").display().to_string()
    )]);
}
...
fn switch_existing_stale_emits_update_stale_then_print() {
    ...
    let plan = plan_switch(&cfg, &args, &obs).expect("plan ok");
    assert_eq!(plan.actions[0], Action::JjWorkspaceUpdateStale { name: "feat-x".into() });
...
fn switch_existing_with_rerun_hooks_reruns_them() {
    let cfg = cfg_with_two_pre_start_groups();
    let args = SwitchArgs { name: "feat-x".into(), create: false, rerun_hooks: true };
    let mut obs = observed_clean();
    obs.workspaces.push(Workspace {
        name: "feat-x".into(),
        path: PathBuf::from("/repo/.worktrees/feat-x"),
